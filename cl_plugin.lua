...
function PLUGIN:DrawDoorInfo(door, width, position, angles, scale, clientPosition)
	...
	local level = door:GetNetVar("level") or 0
	local price = door:GetNetVar("customprice") or math.Round(door:GetNetVar("price", ix.config.Get("doorCost")) * ix.config.Get("doorSellRatio"))
	...
	elseif (!text) then
		if SimpleXPGetLevel(LocalPlayer()) >= level or nil then
			text = door:GetNetVar("ownable") and ("Press F2 To Purchase - " .."( $" ..price.." )")
		else
			text = door:GetNetVar("ownable") and ("You must be at least Level " ..door:GetNetVar("level") .." to purchase this property")
		end
	end
	...
	-- description
	if door:GetNetVar("ownable") then
